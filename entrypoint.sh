...
'deploy-cluster')
    ...
    echo Applying secret-backrest.yaml in ${KUBERNETES_NAMESPACE} namespace
    kubectl_token apply -f /operator/secret-backrest.yaml
	kubectl_token apply -f /operator/secret-pgo-user.yaml
    sleep 10

    # Apply RBAC
    echo Applying rbac.yaml in ${KUBERNETES_NAMESPACE} namespace
    kubectl_token apply -f /operator/rbac.yaml
	
    sleep 10

    # Apply Operator
    echo Applying deployment.yaml in ${KUBERNETES_NAMESPACE} namespace
    kubectl_token apply -f /operator/deployment.yaml
    sleep 10

    # Apply service
    echo Applying service.yaml in ${KUBERNETES_NAMESPACE} namespace
    kubectl_token apply -f /operator/service.yaml
    sleep 10

    # Apply config map
    echo Applying configmap.yaml in ${KUBERNETES_NAMESPACE} namespace
    kubectl_token create configmap pgo-config --from-file=/operator/conf --from-file=/operator/pgo.yaml
    sleep 10

	# Apply pgo-client map
    echo Applying pgo-client.yaml in ${KUBERNETES_NAMESPACE} namespace
    kubectl_token apply -f /operator/pgo-client.yaml
    sleep 10
